    ...
    ILogger<GccV2FirstPartySkillSeeder> logger) : IHostedService
{
        ...
        try
        {
            ...
            foreach (var skill in GccV2SkillCatalog.Definitions.OrderBy(x => x.Order))
            {
                if (existing.Any(x => x.Slug == skill.Id
                    ...
                    continue;
                var skillText = PlainText(skill);
                var fileDigest = Hash(skillText);
                var command = new ImportGccV2SkillCommand(
                    ...
                    true, false,
                    [new("SKILL.txt", "text/plain", Encoding.UTF8.GetByteCount(skillText), fileDigest, skillText)],
    ...
    private static string PlainText(GccV2SkillDefinition skill) => $"""
        Name: {skill.Id}
