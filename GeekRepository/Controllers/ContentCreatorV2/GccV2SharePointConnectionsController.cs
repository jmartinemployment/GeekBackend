...
[ApiController]
[Route("repo/content-creator-v2/sharepoint/connections")]
[Authorize(Policy = RepositoryAuthConstants.InternalServicePolicy)]
public sealed class GccV2SharePointConnectionsController(ContentCreatorV2DbContext db) : ControllerBase
{
    [HttpGet]
    public async Task<ActionResult<IReadOnlyList<GccV2SharePointConnection>>> List(
        [FromQuery] string ownerUserId, CancellationToken ct)
    {
        ...
        var rows = await db.GccV2SharePointConnections.AsNoTracking()
    ...
    [HttpGet("{id:guid}")]
    public async Task<ActionResult<GccV2SharePointConnection>> Get(
        Guid id, [FromQuery] string ownerUserId, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(ownerUserId)) return BadRequest("ownerUserId is required.");
        var row = await db.GccV2SharePointConnections.AsNoTracking()
            .SingleOrDefaultAsync(x => x.Id == id && x.OwnerUserId == ownerUserId, ct);
        return row is null ? NotFound() : Ok(row);
    }

    [HttpPost]
    public async Task<ActionResult<GccV2SharePointConnection>> Upsert(UpsertCommand command, CancellationToken ct)
    {
    ...
    {
        if (string.IsNullOrWhiteSpace(ownerUserId)) return BadRequest("ownerUserId is required.");
        var row = await db.GccV2SharePointConnections
            .SingleOrDefaultAsync(x => x.Id == id && x.OwnerUserId == ownerUserId, ct);
