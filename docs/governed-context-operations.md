...
- Connector SPI ships with the `url` connector (`POST …/knowledge/from-url`; HTTP first with mobile Playwright fallback when extraction is thin/empty), the `gsc` connector (`POST …/knowledge/from-gsc`, owner-owned Search Console connections only), and the `drive` connector (`POST …/knowledge/from-drive`, owner-owned Google Drive connections, approved media types only; image/audio/video remain fail-closed). Run-scoped temporary attachments also accept `POST …/creates/{createId}/attachments/from-url` (same hydrate path, 30-day retention). SharePoint remains out until Microsoft owner-authenticated service boundaries are approved. Pipeline `canvas` handoffs now attach real Canvas assets from prior TaskRun artifacts; `publish` marks ready-to-publish without an external CMS.
