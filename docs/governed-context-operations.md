...
- Connector SPI ships with the `url` connector (`POST …/knowledge/from-url`; HTTP first with mobile Playwright fallback when extraction is thin/empty) and the `gsc` connector (`POST …/knowledge/from-gsc`, owner-owned Search Console connections only). Run-scoped temporary attachments also accept `POST …/creates/{createId}/attachments/from-url` (same hydrate path, 30-day retention). Drive/SharePoint remain out until owner-authenticated service boundaries are approved. Pipeline `canvas` handoffs now attach real Canvas assets from prior TaskRun artifacts; `publish` marks ready-to-publish without an external CMS.
