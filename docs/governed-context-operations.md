...
- Image OCR is available only when a local CLI is configured (`GEEK_CC_OCR_COMMAND`, typically Tesseract) and `GEEK_CC_OCR_DATA_PROCESSING_APPROVED=true`. Audio and video remain fail-closed until local transcription/keyframe models, golden fixtures, resource budgets, and data-processing approval are configured. They must not be routed to a hosted parser.
- Connector SPI ships with the `url` connector (`POST …/knowledge/from-url`; HTTP first with mobile Playwright fallback when extraction is thin/empty), the `gsc` connector (`POST …/knowledge/from-gsc`, owner-owned Search Console connections only), the `drive` connector (`POST …/knowledge/from-drive`, owner-owned Google Drive connections, approved media types only; images require local OCR; audio/video remain fail-closed), and the `sharepoint` connector (`POST …/knowledge/from-sharepoint`, owner-owned Microsoft Graph SharePoint/OneDrive connections, approved media types only). Run-scoped temporary attachments also accept `POST …/creates/{createId}/attachments/from-url` (same hydrate path, 30-day retention). Pipeline `canvas` handoffs now attach real Canvas assets from prior TaskRun artifacts; `publish` marks ready-to-publish without an external CMS.
