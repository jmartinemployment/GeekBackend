        ...
        foreach (var item in items.OrderBy(i => i.RankAbsolute))
        {
            ...
            if (string.Equals(item.Type, AiOverview, StringComparison.OrdinalIgnoreCase))
            {
                serp.Add(new ContentWriterSerpItem
                {
                    Position = position++,
                    Type = AiOverview,
                    // The upstream AI Overview body column is deliberately not read here: this
                    // snippet lands in prompt assembly for the content writer, and that column's
                    // format is not an interchange format at any hop, prompt assembly included.
                    // The status message and description are already plain text.
